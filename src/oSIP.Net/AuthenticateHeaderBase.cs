...
namespace oSIP.Net
{
    public abstract unsafe class AuthenticateHeaderBase : OwnershipDisposable
    {
        ...
        protected static bool TryParse<T>(string str, out T header) where T : AuthenticateHeaderBase, new()
        {
            return TryParseCore(str, out header).EnsureSuccess(ref header);
        }

        private static ErrorCode TryParseCore<T>(string str, out T header) where T : AuthenticateHeaderBase, new()
        {
            var strPtr = Marshal.StringToHGlobalAnsi(str);
            try
            {
                header = new T();
                return NativeMethods.osip_www_authenticate_parse(header._native, strPtr);
            }
            finally
            {
