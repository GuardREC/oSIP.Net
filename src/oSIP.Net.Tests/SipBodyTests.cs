...
namespace oSIP.Net.Tests
{
    ...
    public class SipBodyTests
    {
        ...
        {
            using (var body = SipBody.Parse("abc"))
            {
        ...
        {
            using (var body = SipBody.ParseMime("content-type: text/plain\r\n\r\nabc"))
            {
                Assert.That(body.Data, Is.EqualTo("abc"));
        ...
        {
            using (var original = SipBody.Parse("abc"))
